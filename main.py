...
class BertForNer:
    ...
    def train(self):
        ...
        for epoch in range(self.args.train_epochs):
            for step, batch_data in enumerate(self.train_loader):
                ...
                # 通过调用loss.backward()计算梯度（gradient）并反向传播误差。
                loss.backward()
                # 使用torch.nn.utils.clip_grad_norm_()函数来限制梯度的范数（norm）
                torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.args.max_grad_norm)
                # 使用优化器（optimizer）更新模型（model）的参数（parameters）。
                self.optimizer.step()
                # 使用学习率调度器（scheduler）调整学习率（learning rate）
                self.scheduler.step()
                # 将模型的梯度（gradient）归零（zero_grad）以进行下一轮训练
                self.model.zero_grad()
    ...
    def dev(self):
        ...
        with torch.no_grad():
            ...
            for eval_step, dev_batch_data in enumerate(self.dev_loader):
                ...
                dev_loss, dev_logits = self.model(dev_batch_data['token_ids'], dev_batch_data['attention_masks'],
                ...
                if self.args.use_crf == 'True':
                    ...
                    # 则首先将模型的输出结果从GPU中detach并转移到CPU上，并将其转换为numpy数组。
                    # 然后使用numpy.argmax方法在第二个维度（axis=2）上寻找最大值的索引，并将结果转换为Python列表，最终赋值给batch_output。
    ...
    def predict(self, raw_text, model_path):
        ...
        with torch.no_grad():
            ...
            try:
                attention_masks = torch.from_numpy(np.array(encode_dict['attention_mask'], dtype=np.uint8)).unsqueeze(
                    0).to(device)
            except Exception as e:
                attention_masks = torch.from_numpy(np.array(encode_dict['attention_mask'])).long().unsqueeze(0).to(
                    device)
